...
import { requireSupabaseAuth, assertNoSpoofedUserId } from '../middleware/supabaseAuth.js';
import {
  validateRequest,
...
export function createVideoRouter(): Router {
  ...
  router.post(
    '/api/generate-videos',
    expensiveLimiter,
    ...creditGate('generateVideo'),
    validateRequest(veoVideoGenerationSchema),
    async (req, res) => {
      try {
  ...
  // Poll status — auth wymagany (bez ponownego pobierania kredytów)
  router.post(
    '/api/get-videos-operation',
    requireSupabaseAuth,
    assertNoSpoofedUserId,
    validateRequest(getVideosOperationSchema),
    async (req, res) => {
      try {
