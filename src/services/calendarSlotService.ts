...
export function navigateToCalendarSlot(
  ...
  const prefill = buildPrefillFromCalendarSlot(item);
  const params = new URLSearchParams();
  if (prefill.topic) params.set('topic', prefill.topic);
  // W Next.js reszta prefilli (np. tone, generationType) może być obsługiwana przez Zustand
  // ale topic wystarczy, by generator załadował dane wejściowe, lub możemy rozszerzyć params.
  if (prefill.audience) params.set('audience', prefill.audience);
