  ...
  return (
    <section>
      ...
      {sortedPosts.length === 0 && (
        <div className="rounded-xl border border-dashed border-slate-200 dark:border-slate-700 bg-slate-50/80 dark:bg-slate-900/40 p-4 text-center space-y-3">
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Twoje zaplanowane posty pojawią się tutaj.
          </p>
          <button
            type="button"
            onClick={() => navigate('/calendar')}
            className="inline-flex items-center justify-center gap-2 w-full px-3 py-2.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white text-sm font-bold transition-colors"
          >
                ...
                >
                  <div className="flex items-start flex-grow min-w-0 pr-4">
                    ...
                    <div className="flex-grow min-w-0">
                      <p className="font-semibold text-sm text-slate-900 dark:text-white truncate" title={post.formData?.topic || ''}>{post.formData?.topic || 'Bez tytułu'}</p>
