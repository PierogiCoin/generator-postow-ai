...
import { CalendarFillToolbar } from '@/components/calendar/CalendarFillToolbar';
import { DayDetailDrawer } from '@/components/calendar/DayDetailDrawer';
import { BulkQueuePublisherModal } from '@/components/calendar/BulkQueuePublisherModal';
...
import { IntelligenceGapStrip } from '@/components/intelligence/IntelligenceGapStrip';
...
export function CalendarClient() {
  ...
  const handleUseSuggestionWithSlot = async (suggestion: CalendarSuggestion, autoGenerate = true) => {
    ...
    await setIntelligentCalendarPlan(merged);
    closeDay();
    navigateToCalendarSlot(item, (url) => router.push(url), autoGenerate);
  };
  ...
  return (
    <div className="border border-slate-200/80 dark:border-white/10 bg-white/70 dark:bg-[#0a1220]/70 p-6 md:p-8 animate-fade-in relative overflow-hidden rounded-2xl">
      <ConfirmDialog {...confirmDialogProps} />

      <div className="content-auto-sm space-y-4">
      <CalendarFillToolbar
        presetId={presetId}
        weekTheme={weekTheme}
        platform={platform}
      ...
      <IntelligenceGapStrip
        ...
        onSelectSlot={(slot: GapSlotResult) => void handleApplyGapSlot(slot)}
      />
      </div>
