...
function TrendsContent() {
  ...
  const initialNiche =
    searchParams?.get('prefillNiche')?.trim() ||
  ...
  useEffect(() => {
    const fromNav = searchParams?.get('prefillNiche')?.trim();
  ...
  const onGenerateFromTrend = (topic: string, hashtags: string[]) => {
    ...
    if (pack) {
      ...
      const prefill = industryPackToFormPrefill(pack, fullTopic, audience);
      if (prefill.topic) queryParams.set('topic', prefill.topic);
      if (prefill.audience) queryParams.set('audience', prefill.audience);
    } else {
      queryParams.set('topic', fullTopic);
      if (nicheValue) queryParams.set('audience', nicheValue);
    }
  ...
  return (
    <div className="page-shell-narrow">
      ...
      <div className="p-6 border border-slate-200/80 dark:border-white/10 bg-white/70 dark:bg-[#0a1220]/70 rounded-2xl">
      ...
      <div className="grid-auto-fit-lg">
        {trends.map((trend) => (
          <div key={trend.topic} className="content-auto-card min-w-0">
            <TrendCard
              trend={trend}
...
export function TrendsClient() {
    return (
        <Suspense fallback={<div className="p-8 text-center text-slate-500">Ładowanie trendów...</div>}>
            <TrendsContent />
        </Suspense>
    );
}
