...
export const ABTestResultDisplay: React.FC<{
  result: GenerationResult;
  onUpdateResult: (r: GenerationResult) => void;
  onOpenCreativeStudio: () => void;
}> = ({ result: initialResultData, onUpdateResult, onOpenCreativeStudio }) => {
  const notificationSystem = useNotifications();
  useAppHandlers(notificationSystem.addToast, notificationSystem.addNotification);
  ...
  const variantIdsKey = resultData.variants?.map((v) => v.id).join('|') ?? '';
  const perfKey =
    resultData.variants
      ?.map((v) =>
        v.performance
          ? `${v.id}:${v.performance.reach}:${v.performance.likes}:${v.performance.comments}:${v.performance.shares}`
          : v.id
      )
      .join('|') ?? '';
  ...
  useGSAP(
      ...
      mm.add(
        ...
        (context) => {
          if (context.conditions?.reduceMotion) {
            gsap.set('.ab-header, .ab-variant', { autoAlpha: 1, y: 0 });
            return;
          }

          const tl = gsap.timeline({ defaults: { ease: 'power2.out' } });
          tl.from('.ab-header', { autoAlpha: 0, y: 24, duration: 0.55 }).from(
            '.ab-variant',
            { autoAlpha: 0, y: 32, duration: 0.6, stagger: 0.14 },
            '-=0.25'
          );
        }
      );
    ...
    { scope: rootRef, dependencies: [variantIdsKey] }
  );

  useGSAP(
    () => {
      const hasPerf = resultData.variants?.some((v) => v.performance);
      if (!hasPerf) return;

      const mm = gsap.matchMedia();
      mm.add('(prefers-reduced-motion: no-preference)', () => {
        const nodes = gsap.utils.toArray<HTMLElement>('.ab-stat-value');
        nodes.forEach((el) => {
          ...
          gsap.to(state, {
            ...
            ease: 'power2.out',
            onUpdate: () => {
              el.textContent = Math.round(state.value).toLocaleString('pl-PL');
            },
          });
        });
      });
    ...
    { scope: rootRef, dependencies: [perfKey] }
  );
  ...
  const getPerformanceScore = (p?: PostPerformanceData) =>
    !p ? 0 : p.reach * 0.1 + p.likes + p.comments * 2 + p.shares * 3;
  ...
  const potentialWinnerId =
    hasPerformanceData && scores[0] !== scores[1]
      ? scores[0] > scores[1]
        ? resultData.variants![0].id
        : resultData.variants![1].id
      : null;

  return (
    <div ref={rootRef} className="space-y-10">
      <div className="ab-header relative p-10 bg-slate-50 dark:bg-slate-900/50 rounded-[2.5rem] border-2 border-slate-100 dark:border-slate-800 overflow-hidden text-center">
        ...
        <div className="relative z-10 space-y-4">
          ...
          <p className="max-w-xl mx-auto text-slate-500 dark:text-slate-400 font-medium">
            Testujemy dwa warianty Twojej treści, aby sprawdzić, który z nich osiągnie najlepsze rezultaty u
            odbiorców.
          </p>
          {!hasPerformanceData && (
            <div className="pt-4">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 xl:gap-8 min-w-0">
        {resultData.variants?.map((variant, index) => {
          ...
          return (
            <ModernCard
              key={variant.id}
              className={`ab-variant p-1 relative overflow-hidden transition-all duration-500 h-full flex flex-col min-w-0 ${
                isWinner
                  ? 'ring-4 ring-amber-400 dark:ring-amber-500 shadow-2xl shadow-amber-500/20 scale-[1.02] z-10'
                  : isPotentialWinner
              ...
              <div className="p-6 pb-2 flex justify-between items-center">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-black text-slate-900 dark:text-white uppercase tracking-widest text-xs">
                    Wariant {String.fromCharCode(65 + index)}
                  </span>
                </div>
                {isWinner && (
                  <div className="flex items-center gap-2 bg-amber-100 dark:bg-amber-900/30 px-3 py-1 rounded-full border border-amber-200 dark:border-amber-800/50">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    {[
                      ...
                      { icon: HeartIcon, label: 'Lubi to', value: variant.performance.likes, color: 'text-rose-500' },
                      {
                        icon: ChatBubbleIcon,
                        label: 'Koment.',
                        value: variant.performance.comments,
                        color: 'text-emerald-500',
                      },
                      {
                        icon: ShareIcon,
                        label: 'Udostępn.',
                        value: variant.performance.shares,
                      ...
                      <div key={label} className="flex flex-col gap-1">
                        ...
                        <div className="ab-stat-value text-xl font-black text-slate-800 dark:text-slate-200" data-value={value}>
