...
export const InputForm: React.FC<InputFormProps> = ({
  ...
  return (
    <>
      ...
      <div className="sm:hidden fixed bottom-[max(4.2rem,calc(env(safe-area-inset-bottom)+3.8rem))] left-0 right-0 z-30 px-4 pointer-events-none">
        <div className="pointer-events-auto max-w-lg mx-auto bg-slate-900/95 dark:bg-[#071018]/95 backdrop-blur-xl border border-white/15 rounded-2xl p-2.5 shadow-2xl flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 pl-2 min-w-0">
            <SparklesIcon className="w-5 h-5 text-sky-400 shrink-0" />
            <span className="text-xs font-bold text-white truncate">
              {isLoading ? t('common.generating') : t('form.stickyMobilePrompt', 'Gotowy na post?')}
            </span>
          </div>
          <button
            type="button"
            onClick={() => {
              const formEl = document.getElementById('input-form-anchor')?.querySelector('form');
              if (formEl) formEl.requestSubmit();
            }}
            disabled={isLoading}
            className="px-5 py-2.5 rounded-xl text-xs font-extrabold text-white bg-[var(--hero-accent)] hover:brightness-110 active:scale-95 shadow-md transition-all shrink-0 flex items-center gap-1.5 disabled:opacity-50"
          >
