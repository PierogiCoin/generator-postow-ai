    ...
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      {step === 1 && (
        <div className="space-y-4 animate-fade-in">
          ...
          <div className="rounded-2xl border-2 border-slate-200/90 dark:border-white/10 bg-white/90 dark:bg-slate-900/60 backdrop-blur-xl focus-within:border-[var(--hero-accent)] focus-within:ring-4 focus-within:ring-[var(--hero-accent)]/20 shadow-lg shadow-slate-900/5 transition-all duration-300 relative overflow-hidden p-1">
            <InteractiveEditor
              id="topic-quick"
              ...
              isLoading={isLoading}
              formData={formData}
              className="w-full text-sm sm:text-base font-medium min-h-[140px]"
              lite
            />
            <div className="flex items-center justify-between px-3 py-1.5 bg-slate-50/80 dark:bg-white/[0.02] border-t border-slate-100 dark:border-white/5 rounded-b-xl text-[10px] font-semibold text-slate-400">
              <span className="flex items-center gap-1 text-[var(--hero-accent)]">
                <SparklesIcon className="w-3.5 h-3.5" />
                {t('form.topic.heroHint', 'Wpisz opisu lub użyj asystenta AI')}
              </span>
              <span>{topicPlain.length} znaków</span>
            </div>
          </div>
