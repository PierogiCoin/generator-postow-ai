...
export const useGenerate = ({ addToast, t, handleApiError }: GenerationHandlerDeps) => {
    ...
    const handleGenerate = useCallback(async (rawFormData: FormData) => {
        ...
        try {
            ...
            const generationInsights = [...(learnedInsights || []), ...(formData.learnedInsights || [])];
    ...
    return { handleGenerate, abortControllerRef };
};
