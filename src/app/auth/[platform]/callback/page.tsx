...
export const SocialAuthCallback: React.FC = () => {
    ...
    useEffect(() => {
        const processCallback = async () => {
            ...
            // Pobieranie kodów z URL używając Next.js searchParams
        ...
        processCallback();
    }, [user, platform, searchParams, router]);
            ...
            <div className="p-8 bg-white dark:bg-slate-800 shadow-xl rounded-2xl max-w-md w-full text-center">
                {error ? (
                    <div>
                        ...
                        <button onClick={() => router.push('/dashboard')} className="px-6 py-2 bg-slate-900 text-white rounded-lg hover:bg-slate-800 transition-colors">
                            Wróć do panelu
                        </button>
                    </div>
                ) : (
