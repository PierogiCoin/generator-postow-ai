...
import type { CalendarSlotContext, FormData } from '@/types';
import { NotificationType } from '@/types';

// Components
import { GeneratorSidebar } from '@/components/generator/GeneratorSidebar';
import { InputForm } from '@/components/InputForm';
import { StrategyAssistant } from '@/components/dashboard/StrategyAssistant'; // Reused
...
const ResultCard = dynamic(() => import('@/components/ResultCard').then(m => m.ResultCard), { ssr: false, loading: () => <SkeletonCard /> });
const MultiPlatformOptimizer = dynamic(() => import('@/components/MultiPlatformOptimizer').then(m => m.MultiPlatformOptimizer), { ssr: false, loading: () => <SkeletonCard /> });
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 xl:gap-8 min-h-[calc(100vh-140px)] relative bg-transparent animate-fade-in pb-16">
      ...
      <div className="flex-grow relative z-10 min-w-0">
          ...
          <div className={`w-full transition-all duration-300 ${isResultVisible && !isMobile ? 'grid gap-6 xl:gap-8 lg:grid-cols-2 items-start' : ''}`}>
              ...
              {showResultColumn && (
                  <div id="generation-result" className="space-y-5 animate-fade-in lg:sticky lg:top-8 self-start">
                      {isLoading ? (
                          ...
                          <Suspense fallback={<SkeletonCard />}>
                              <ResultCard historyResult={('result' in (inspiration || {})) ? (inspiration as any).result : null} />
                          </Suspense>
                      )}
