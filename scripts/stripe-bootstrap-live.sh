#!/usr/bin/env bash
# Tworzy produkty/ceny Generator Postów AI + webhook produkcyjny (Stripe LIVE mode przez CLI).
set -euo pipefail
...
find_price_by_interval() {
  local product_id="$1"
  local interval="$2"
  stripe --live prices list --product "$product_id" --active=true --limit 100 \
    | python3 -c "
import sys, json
interval = sys.argv[1]
...
create_sub() {
  ...
  local existing
  existing=$(stripe --live products search --query "metadata['app']:'${APP_META}' AND metadata['plan_key']:'${plan_key}'" --limit 1 2>/dev/null | python3 -c "import sys,json; d=json.load(sys.stdin); print(d['data'][0]['id'] if d.get('data') else '')" || true)
  ...
  if [[ -n "$existing" ]]; then
    ...
    product_id=$(stripe --live products create \
      -d "name=${name}" \
...
create_pack() {
  ...
  local price_id
  price_id=$(stripe --live prices list --product "$product_id" --active=true --limit 1 \
...
echo ""
echo "=== Webhook ==="
WEBHOOK_JSON=$(stripe --live webhook_endpoints list --limit 100)
...
if [[ -n "$EXISTING_WE" ]]; then
  ...
  WE_JSON=$(stripe --live webhook_endpoints create \
    --url="$WEBHOOK_URL" \
